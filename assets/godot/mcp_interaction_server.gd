...
func _ready() -> void:
	...
	var err: int = _server.listen(listen_port, "127.0.0.1")
	if err != OK:
		push_error("McpInteractionServer: Failed to listen on port %d, error: %d" % [listen_port, err])
		return
...
func _cmd_set_shader_param(params: Dictionary) -> void:
	...
	if material == null:
		var auto_shader := Shader.new()
		auto_shader.code = "shader_type spatial;\n\nvoid fragment() { ALBEDO = vec3(1.0); }\n"
		var auto_mat := ShaderMaterial.new()
		auto_mat.shader = auto_shader
		if node.has_method("set_surface_override_material"):
			node.set_surface_override_material(0, auto_mat)
		elif node.has_method("set_material"):
			node.set_material(auto_mat)
		else:
			node.set("material_override", auto_mat)
		material = auto_mat
	...
	match action:
		"get_source":
			...
			_send_response({
				"success": true,
				"has_script": true,
				"language": "gdscript" if s is GDScript else "external",
				"source": s.source_code if s is GDScript else "",
		...
		"attach":
			...
			if not script_path.is_empty():
				...
				var loaded_script = load(full_script_path)
				if loaded_script == null:
					_send_response({"error": "Failed to load script from script_path: %s" % full_script_path})
					return
			...
			if source.is_empty():
				_send_response({"error": "source or script_path is required for attach"})
				return
...
func _cmd_visual_shader(params: Dictionary) -> void:
	...
	match action:
		"create":
			...
			mesh.name = params.get("name", "VisualShaderMesh")
			...
			_send_response({"success": true, "action": "create", "path": str(mesh.get_path())})
		"info":
			_send_response({"success": true, "action": "info"})
		_:
...
func _cmd_terrain(params: Dictionary) -> void:
	...
	match action:
		"create":
			...
			mesh_instance.name = params.get("name", "Terrain")
			var plane := PlaneMesh.new()
			plane.size = Vector2(float(params.get("size_x", 20.0)), float(params.get("size_z", 20.0)))
			mesh_instance.mesh = plane
			parent.add_child(mesh_instance)
			_send_response({"success": true, "action": "create", "path": str(mesh_instance.get_path())})
		_:
...
func _cmd_video(params: Dictionary) -> void:
	...
	match action:
		"create":
			...
			player.name = params.get("name", "VideoPlayer")
